...
this.s4.dx = fnder(this.s4,[1,0]);
this.s4.dy = fnder(this.s4,[0,1]);
    ...
    for j = 1:length(Y)
        if X(i)==0 && Y(j) == 0
            ...
            Vtv = Minv*a;
             ...
             Vt1 = G*Vconv/norm(Vconv)+H*Vcirc/norm(Vcirc);
...
%             Vt1 = G*Vconv+H*Vcirc;
            ...
            Vtv = Minv*a;
             ...
             Vt2 = G*Vconv/norm(Vconv)+H*Vcirc/norm(Vcirc);
%             Vt2 = G*Vconv+H*Vcirc;

            %Repulsive Field 3
            dx3 = fnval(this.s4.dx,[X(i),Y(j)]');
            dy3 = fnval(this.s4.dy,[X(i),Y(j)]');
            dz3 = 0;
            crs = cross([dx3;dy3;0],[0;0;1]);
            Vconv = -fnval(this.s4,[X(i),Y(j)]')*[dx3;dy3;dz3];
            Vcirc = -crs;
            a1 = diff([fnval(this.s4,[X(i),Y(j)]'),fnval(this.s4,[X(i),Y(j)]')])/dt;
            a2 = 0;
            a3 = 0;
            a = [a1;a2;a3];
            M = [dx3',dy3',0;0,0,1;crs'];
            Minv = inv(M);
            Vtv = Minv*a;
            
            Vt3 = G*Vconv/norm(Vconv)+H*Vcirc/norm(Vcirc);
...
plot(x3(1:end-1),y3(1:end-1),'r','linewidth',2)
axis equal
